    ...
    <>
      <section className="hero">
        <div className="container hero-inner">
          ...
          <div className="hero-proof"><span><strong>{seedResources.length}+</strong> {t("curatedResources")}</span><span><strong>5</strong> {t("focusedCategories")}</span><span><strong>100%</strong> {t("freeToBrowse")}</span></div>
        </div>
      </section>
            ...
            {categories.map((category) => {
              ...
              return <article className="inside-column" key={category.slug}>
                ...
                <div className="inside-links">
                  {categoryResources.map((resource) => <a href={resource.url} target="_blank" rel="noopener noreferrer" key={resource.id}><span><strong>{resource.id === "seed-4" ? "SimplifyJobs Summer 2026 Internships" : resource.title}</strong><small>{subcategoryName(resource.subcategory)}</small></span><ExternalLink size={14} /></a>)}
                </div>
