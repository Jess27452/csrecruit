...
export async function getApprovedResources(options: { category?: string; limit?: number } = {}): Promise<Resource[]> {
  ...
  let query = supabase.from("resources").select("id,title,description,url,category,subcategory,tags,submitted_by,status,created_at,profiles!resources_submitted_by_fkey(username,avatar_url)").eq("status", "approved").order("created_at", { ascending: false });
  ...
  return data.map((row: Record<string, unknown>) => {
    ...
    return { id: String(row.id), title: String(row.title), description: String(row.description), url: String(row.url), category: String(row.category), subcategory: String(row.subcategory), tags: Array.isArray(row.tags) ? row.tags.map(String) : [], submittedBy: (profile as { username?: string } | null)?.username || "Community member", submittedById: String(row.submitted_by || ""), avatarUrl: (profile as { avatar_url?: string } | null)?.avatar_url, createdAt: String(row.created_at), upvotes: 0, status: "approved" };
  });
}
